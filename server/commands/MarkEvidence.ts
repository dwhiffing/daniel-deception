...
export class MarkEvidenceCommand extends Command<
  RoomState,
  {
    playerId: string
    type: string
...
> {
  ...
  execute({ playerId, type, value }) {
    ...
    const markedCardsLength = this.state.sceneDeck.filter(
      (c) => c.markedValueIndex > -1,
    ).length
    ...
    if (markedCardsLength === cardsThisRound) {
      ...
      this.state.phaseTimer =
        this.state.phaseTimerMultiple * (this.state.players.length - 1)
      // TODO: discard the unpicked cards
